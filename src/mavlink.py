...
os.environ["LD_LIBRARY_PATH"] = (
    f"{os.path.join(ghost_sdk_path, 'lib')}:{os.environ.get('LD_LIBRARY_PATH', '')}"
)
...
class MB80v2(MBLink):
    ...
    def __init__(
        self, sim=False, verbose=True, log=False, port=0, use_blocking_get=True
    ):
        ...
        if log:
            self.startLog()
        self.start(
            "127.0.0.1" if sim else "192.168.168.105", verbose, port, use_blocking_get
        )
        self.rxstart()
...
class GhostRobotClient:
    def __init__(self, sim=True):
        ...
        self.twist_values = np.zeros(
            4, dtype=np.float32
        )  # x, y, z, r (forward, lateral, rotation, height)
    ...
    def set_action_mode(self, mode):
        ...
        try:
            ...
            logger.info(
                f"Robot reports action mode: {actual_mode}, internal tracking: {self._action_mode}"
            )
    ...
    def get_action_mode(self):
        ...
        try:
            ...
            if actual_mode != self._action_mode:
                logger.debug(
                    f"Mode discrepancy - robot:{actual_mode}, internal:{self._action_mode}"
                )
    ...
    def move_robot(self, x=0.0, y=0.0, z=0.0, r=0.0, duration=0.0):
        ...
        """
        logger.info(
            f"move_robot called with x={x}, y={y}, z={z}, r={r}, duration={duration}"
        )
        ...
        try:
            ...
            self.twist_values = self.twist_values + filt_coeff * (
                target_twist - self.twist_values
            )
                ...
                try:
                    while time.time() < end_time:
                        ...
                        if elapsed < accel_time:
                            ...
                            factor = np.sin(
                                ((duration - elapsed) / accel_time) * (np.pi / 2)
                            )
                        else:
                            factor = 1.0
    ...
    def roll_over(self, mode=1):
        ...
        try:
            logger.info(
                f"Setting roll over mode from {self.mb.getMode('roll_over')} to {mode}"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        client.move_robot(
            x=0.0, y=0.5, z=0.0, r=0.0, duration=2.0
